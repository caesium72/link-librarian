...
function buildGraph3D(links: Link[], theme: "cosmos" | "atomic" | "sphere" | "ocean" | "galaxy") {
...
export function KnowledgeGraph3D({ links, isLoading, theme = "cosmos" }: KnowledgeGraph3DProps) {
  ...
  // Feature states
  const [activeFeature, setActiveFeature] = useState<"none" | "search" | "time" | "path"  | "stats"| "cluster">("none");
      ...
      <Card className="overflow-hidden border-primary/10 hover:border-primary/20 transition-colors duration-300">
        <CardContent className="p-0 relative">
          <div className="h-[500px] w-full bg-background">
            <Canvas
              camera={{ position: theme === "atomic" ? [0, 6, 20] : theme === "sphere" ? [0, 10, 28] : theme === "ocean" ? [0, 5, 25] : theme === "galaxy" ? [0, 12, 22] : [0, 8, 25], fov: 50 }}
              dpr={[1, 2]}
